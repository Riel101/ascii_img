...
func asciiToPNG(ascii string) ([]byte, error) {
	...
	draw.Draw(img, img.Bounds(), &image.Uniform{C: color.RGBA{40, 40, 40, 255}}, image.Point{}, draw.Src)
...
func convertImage(r io.Reader, targetW int) (string, string, error) {
	img, err := loadImg(r)
	if err != nil {
	...
	return ascii, base64.StdEncoding.EncodeToString(pngBytes), nil
}
...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	ascii, b64, err := convertImage(file, 120)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
